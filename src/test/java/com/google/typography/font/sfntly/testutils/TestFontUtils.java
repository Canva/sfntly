...
public class TestFontUtils {
   ...
   * @param path the path to the font(s) resource
   ...
   * @throws IOException
   */
  private static Font[] loadFont(String path, boolean fingerprint) throws IOException {
    FontFactory fontFactory = FontFactory.getInstance();
   ...
   * @throws IOException
   */
  public static Font[] loadFont(String path, byte[]... expectedFingerprints) throws IOException {
    Font[] fonts = loadFont(path, true);
    for (int i = 0; i < Math.min(fonts.length, expectedFingerprints.length); i++) {
      if (expectedFingerprints[i] != fonts[i].digest()) {
        throw new IOException("Did not get the expected fingerprint for font#" +
            i + " in " + path + " Has the file changed?");
      }
    }
    return fonts;
  }
   ...
   * @throws IOException
   */
  public static Font[] loadFontUsingByteArray(String file) throws IOException {
   ...
   * @throws IOException
   */
  private static Font[] loadFontUsingByteArray(String file, boolean fingerprint) throws IOException {
  ...
  private static File serializeFont(Font font, File serializedFontFile) throws Exception {
  ...
  public static Builder builderForFontFile(String fontFile) throws IOException {
    return builderForFontFile(fontFile, true);
  }

  private static Builder builderForFontFile(String fontFile, boolean fingerprint) throws IOException {
    InputStream is = Thread.currentThread().getContextClassLoader().getResourceAsStream(fontFile);
